...
from arcgis.features import GeoSeriesAccessor as gsa
...
def generate_heatmap_color_bydate(d):
    ...
    #   ['name', 'slug', 'set', 'kind', 'external_i', 'display_na', 'sqmi',
    #   'type', 'name_1', 'slug_1', 'geometry', 'ID', 'Time Stamp', 'Latitude',
    #   'Longitude', 'Risk-Level', 'cat', 'color']
    print('Using covid_risk_heatmap layer ' + "cefd44c7d0d34aae8b33799c22b73f2b")
    heatmap_layer_item = gis.content.get("cefd44c7d0d34aae8b33799c22b73f2b")
    heatmap_layer = heatmap_layer_item.layers[0]
    # heatmap_layer.delete_features(where="True")
    # assert False
    try:
        heatmap_fset = heatmap_layer.query()
    except Exception as e:
        print(e)
        print("Skipping...")
        return

    heatmap_fset_df = heatmap_fset.sdf  # Pandas dataframe

    geom_list = [arcgis.geometry.Geometry.from_shapely(g) for g in merged.geometry]
    dataframe_blank = pd.DataFrame(merged)
    dataframe_blank = dataframe_blank.drop(columns=["geometry"])
    dataframe_blank.spatial.set_geometry(geom_list)

    date_str_list = list(set(merged['Time Stamp']))
    date_str = None
    for a_str in date_str_list  :
        if isinstance(a_str, str):  # Filters out <0>
            date_str = a_str

    set_date = datetime.strptime(date_str, '%m-%d-%Y')

    dataframe_blank.loc[:, 'date'] = set_date

    dataframe_fs = dataframe_blank.spatial.to_featureset()
    # print(dataframe_fs.features)

    heatmap_layer.edit_features(adds=dataframe_fs)
    # assert False
